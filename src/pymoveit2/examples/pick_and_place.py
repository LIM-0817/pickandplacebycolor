...
# 로봇이 움직이거나 callback 함수가 신호를 받을 때 하나의 task는 멈추게 되어있음(MutuallyExclusiveCallbackGroup)
# ReentarantCallbackGroup을 사용하는 경우 여러 task를 한번에 실행 가능
...
class PickAndPlace(Node):
    def __init__(self):
        ...
        self.drop_joints  = [math.radians(-49.0), math.radians(-59.0), math.radians(-78.0),
                            math.radians(-72.0), math.radians(-63.0), math.radians(71.0), math.radians(-105.0)]

        # 그리퍼가 물체 잡을 때
        self.gripper_grip = [0.03, 0.03]
    ...
    def coords_callback(self, msg):
        ...
        try:
            ...
            if color_id == self.target_color:
                ...
                pick_position = [self.target_coords[0], self.target_coords[1], self.target_coords[2] - 0.6]
                ...
                # 5. Close gripper
                self.gripper.move_to_configuration(self.gripper_grip)
                self.gripper.wait_until_executed()
